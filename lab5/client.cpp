...
int recvFileUdp(int clientSocket, char *fileOut)
{
    ...
    while (1) {
        ...
        if (n <= 0) {
            ...
            printf("Count of receiving bytes: %d\n", currentSize);
            currentSize = 0;
            break;
        }
...
int useTcp(char *host, int port)
{
    ...
    serverAddr.sin_addr.s_addr = getServerAddr(host);
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port = htons(port);

    if (connect
        (clientSocket, (struct sockaddr *) &serverAddr,
         sizeof(serverAddr)) < 0) {
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4){
        printf("Use params: client <host> <port> <filename> \n");
        exit(1);
    }
