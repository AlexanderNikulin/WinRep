...
int sendFileSizeUdp(struct sockaddr_in clientAddr, char *fileName)
{
    ...
    return sendto(listener, &msgFileSize[0], buffer_size, 0,
...
int udpListen()
{
    ...
    if ((listener = socket(PF_INET, SOCK_DGRAM, 0)) < 0) { //pf - family for UDP sock_dgram - type of socket
    ...
    int opt = 1;
    setsockopt(listener, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));  // sol - ??, so_reuse -- for using port twice (if error )
